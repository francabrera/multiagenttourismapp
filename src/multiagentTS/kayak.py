...
def start_hotel_search(sid, othercity, checkin_date, checkout_date, guests1, rooms, numres, mode, sortdir, sortkey):
    sid = getsession()
    if sid is None:
        return None
    else:
        url = hostname + apisearch + '&othercity=' + othercity + '&checkin_date=' + checkin_date + '&checkout_date=' + checkout_date + '&guests1=' + guests1 + '&rooms=' + rooms + '&action=doHotel&version=1&_sid_=' + sid
